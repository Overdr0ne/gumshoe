...
** Consult Integration
   This package also includes extentions for peruse* commands with [[https://github.com/minad/consult][Consult]] to provide live previews of markers as you are perusing the backlog. These commands have the same interface described above, just prefixed with `consult-`, e.g. `consult-gumshoe-peruse-in-buffer`.
** Manual Markers
